    ...
    public class CadastroRegistroViewModel : BaseViewModel
    {
        ...
        public async Task RegistrarOcorrencia()
        {
            try
            {
               ...
               if(String.IsNullOrEmpty(DsRegistro))
               {
                    await Application.Current.MainPage.DisplayAlert("Atenção", "Descreva a ocorrencia", "Ok");
               }
               else
               {
                    if(String.IsNullOrEmpty(MtRegistro))
                    {
                        MtRegistro = DateTime.Now.ToString();
                    }
